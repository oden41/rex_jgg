...
public class TJgg {
	...
	public TJgg(TSphereFunction function,
            TPopulation initialPopulation,
            Random rand) {
		fRandom = rand;
	...
	public void doOneIteration() {
		...
		for (int i = 0; i < n + 1; i++) {
			int index = fRandom.nextInt(fpopulation.getNumberOfPopulation());
			while(parentsIndex.contains(index)){
				index = fRandom.nextInt(fpopulation.getNumberOfPopulation());
			}
		...
		Arrays.sort(children, (a,b) -> a.getEvaluationValue() > b.getEvaluationValue() ? 1 : (a.getEvaluationValue() < b.getEvaluationValue()) ? -1 : 0);
